...
import (
	...
	"natrya.co.id/ryanthe/forum/api/mailer"
	"natrya.co.id/ryanthe/forum/api/models"
	"natrya.co.id/ryanthe/forum/api/security"
	"natrya.co.id/ryanthe/forum/api/utils/formaterror"
)
...
func (server *Server) ForgotPassword(c *gin.Context) {
	...
	response, err := mailer.SendMail.SendResetPassword(resetDetails.Email, os.Getenv("EMAIL_FROM"), resetDetails.Token, os.Getenv("APP_ENV"))
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, gin.H{
			"status": http.StatusUnprocessableEntity,
			"error":  err,
